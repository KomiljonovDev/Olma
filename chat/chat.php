			...
			<div class="col">
				<div class="row head p-2">
					<div class="to">
						...
						<h5 id="userFullName"><b>Olma User</b></h5>
					</div>
...
<script type="text/javascript">
	...
	function getMessages() {
		...
		xhr.open('POST', '../api/chat/?do=getMessages',true);
		xhr.onload = () =>{
			if (xhr.status === 200) {
				const result = JSON.parse(xhr.responseText);
				if (result.ok == true) {
					console.log(result);
					document.getElementById('userFullName').innerText = result.user.name;
					if (result.result.length > 0) {
						msg = '';
						result.result.forEach((argument)=>{
							if (argument.user == 'you') {
								msg += `<div class="w-100 my-2" style="clear: both;">
 							<div class="toMe badge bg-white text-dark text-wrap mt-2 p-2" style="max-width: 20rem;font-size: 15px; text-align: left; border-radius: 10px 0 10px 0;">${argument.message}</div>
 						</div>`;
							}else{
